...
const index = () => {
  ...
  useEffect(() => {
    const fetchAllPosts = async () => {
      try {
        const response = await axios.get("http://10.0.2.2:8000/api/all");
  ...
  const handleLikePost = async (postId) => {
    try {
      const response = await axios.post(
        `http://10.0.2.2:8000/api/like/${postId}/${userId}`
      );
        ...
        <Pressable onPress={() => router.push("/home/profile")}>
          <Image
            ...
            source={{
              uri:
                user?.profileImage ||
                "https://th.bing.com/th/id/OIP.1nWRQ7r_1nEVJ6sdz_zwkwHaE8?w=227&h=180&c=7&r=0&o=5&dpr=1.3&pid=1.7",
            }}
          />
        </Pressable>
